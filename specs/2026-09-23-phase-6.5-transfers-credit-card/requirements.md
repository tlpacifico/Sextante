...
| D3 | **`CategoryId` passa a nullable**; `NULL` obrigatório para `Transfer`/`Adjustment`, obrigatório para `Regular` (CHECK na DB). `Guid.Empty` legado → `NULL` na migration. | Acaba com o sentinel `Guid.Empty` do materializer, que hoje faz linhas desaparecerem dos totais. |
